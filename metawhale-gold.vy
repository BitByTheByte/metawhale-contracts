...
interface IReserves:
    ...
    def swapForSecondary() -> bool: nonpayable
    ...
    def reserveAssetTrade() -> address: view
...
reserveManager: public(address[20])
...
def __init__(_name: String[64], _symbol: String[32], _decimals: uint256, _supply: uint256):
    ...
    self.reserve_threshold = convert(0.0025 * deciCalc, uint256)
...
@view
@external
def showReserveManagers() -> address[20]:
...
def inactivityBurn(_address: address) -> bool:
    ...
    assert self.lastIndividualTrade[_address] > self.lastIndividualTrade[_address] + 10518972, "MetaWhale: Addy is still active." #4 months 
...
def _prepReserve() -> bool:
    ...
    secundary_asset_addy: address = IReserves(self.reserve).reserveAssetTrade()
...
def claimReserve() -> bool:
    ...
    ERC20(IReserves(self.reserve).reserveAssetTrade()).transfer(msg.sender, pctofreserve)
...
def bigreset() -> bool:
    ...
    assert self.manager == 0
    newsupply: uint256 = (1000000*10**self.decimals) - self.total_supply - self.dexAllocation
...
def _manageReserve(_caller: address) -> bool:
    ...
    if _caller not in self.reserveManager:
        if self.reservecounter == 20:
            self.reservecounter = 0
        rsv_check: uint256 = self._pctCalc_minusScale(self.total_supply, self.reserve_threshold)
        if self.balanceOf[self.reserve] > rsv_check and self.n_trades > 20 and self.switcher == 1:
            ...
            IReserves(self.reserve).swapForSecondary()
            self.reserveManager[self.reservecounter] = _caller
            self.reservecounter += 1
            self.switcher = 4
            self._mint(_caller, self.incentive)
...
def _transfer(_from: address, _to: address, _value: uint256) -> bool:
    ...
    if self.manager >= 2:
        if _from != self.deployer:
            if self.n_trades <= 100:
                assert _value <= 7000 * 10 ** self.decimals, "Maximum amount allowed is 7000 MTWG until the 100th transaction."
            else:
                pass
        else:
            self.manager = 2
